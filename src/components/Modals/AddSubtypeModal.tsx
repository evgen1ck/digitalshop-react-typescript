...
import {ApiAdminSubtypeUrl, ApiAdminTypeUrl, postAxioser} from "../../lib/queries";
...
export const AddSubtypeModal = ({ onShow, setShow, canLeave, typeName }: AddModalProps) => {
    ...
    async function handleAddClick() {
        ...
        let body = { subtype_name: value }
        postAxioser(ApiAdminSubtypeUrl + "?name=" + typeName, body).then(() => {
            ...
            if (axios.isAxiosError(error)) {
                if (error.response) {
                    if (error.response.status == 409)
                        toast.error("Невозможно добавить, такой подтип уже есть");
                    else
    ...
    return (
        <Modal onShow={onShow} setShow={setShow} canLeave={canLeave} title={"Добавление подтипа"}>
            <RowBlock>
                <div className="inline-flex w-full">
                    <InputWithValidation
                        nameField={"Название подтипа для " + typeName}
